...
let binom k n =
  ...
  in
  if k>=0 && k<=n then factorielle n / (factorielle k * factorielle (n-k) ) (* le problème de la factorielle est qu'elle dépasse la capacité des int très vite (21! > 2^62 - 1) *)
  else 0
...
let rec croissant lst =
  ...
  | [] -> true
  | [h] -> true
  | h::h2::t -> if h <= h2 then croissant (h2::t) else false (* se remplace avantageusement par une expression booléenne *)
...
let compare_tailles lst_1 lst_2 =
  let rec taille lst =            (*On introduit la fonction taille qui renvoie la taille d'une liste*)
    ...
    | [] -> 0
    | _::t -> 1 + taille t
  in (* pourquoi pas List.length ? *)
  if taille lst_1 <= taille lst_2 then if taille lst_1 < taille lst_2 then 1 else 0
  else -1 (* le désavantage de ta version est que tu parcoures les deux listes en entier *)
...
(* aaah, ils sont là : met les tests directement après la fonction, pas à la fin (même si c'est pas comme ça qu'on fera en C, c'est plus clair). Enrichit tes tests avec des "cas de base", des cas particuliers, ou encore en changeant de type *)
